...
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/jquery/jquery-ui.min.js
 */
...
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/Sortable.min.js
 */
...
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/tag-it.min.js
 */
...
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/bootstrap.min.js
 */
/*!
...
return NProgress;});;
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/moment/moment-with-locales.min.js
 */
...
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/moment/moment-timezone-with-data.min.js
 */
//! moment-timezone.js
//! version : 0.3.1
//! author : Tim Wood
...
/*
 *	/home/guest/frappe-bench/apps/frappe-bench/apps/frappe/frappe/public/js/lib/socket.io.min.js
 */
...
var callback=function(data,response_text){if(data.task_id){frappe.socket.subscribe(data.task_id,opts);}
...
frappe.socket.socket=io.connect(frappe.socket.get_host());if(!frappe.socket.socket){console.log("Unable to connect to "+frappe.socket.get_host());return;}
...
this.wrapper.toggleClass("hide-control",!!hide);},collapse:function(hide){if(hide===undefined){hide=!this.body.hasClass("hide");}
...
frappe.ui.set_user_background=function(src,selector,style){if(!selector)selector="#page-desktop";if(!style)style="Fill Screen";if(!src)src=frappe.urllib.get_full_url(frappe.boot.default_background_image);frappe.dom.set_style(repl('%(selector)s { \
...
frappe.provide("frappe.datetime");$.extend(frappe.datetime,{convert_to_user_tz:function(date,format){if(sys_defaults.time_zone){var date_obj=moment.tz(date,sys_defaults.time_zone).utc().utcOffset(moment.user_utc_offset);}else{var date_obj=moment(date);}
return(format===false)?date_obj:date_obj.format(moment.defaultDatetimeFormat);},convert_to_system_tz:function(date,format){if(sys_defaults.time_zone){var date_obj=moment(date).utc().utcOffset(moment.system_utc_offset);}else{var date_obj=moment(date);}
...
var attachment=r.message;opts.callback(attachment,r);$(document).trigger("upload_complete",attachment);},error:function(r){opts.onerror?opts.onerror(r):opts.callback(null,null,r);return;},btn:opts.btn});}}
...
frappe.provide("frappe.ui.toolbar");frappe.ui.toolbar.Toolbar=Class.extend({init:function(){var header=$('header').append(frappe.render_template("navbar",{}));var sidebar=$('.offcanvas .sidebar-left').append(frappe.render_template("offcanvas_left_sidebar",{}));header.find(".toggle-sidebar").on("click",frappe.ui.toolbar.toggle_left_sidebar);header.find(".toggle-navbar-new-comments").on("click",function(){$(".offcanvas").toggleClass("active-right").removeClass("active-left");return false;});$(document).on("notification-update",function(){frappe.ui.toolbar.update_notifications();});$('.dropdown-toggle').dropdown();$(document).trigger('toolbar_setup');$(document).on("page-change",function(){$("header .navbar .custom-menu").remove();});frappe.search.setup();},});$.extend(frappe.ui.toolbar,{add_dropdown_button:function(parent,label,click,icon){var menu=frappe.ui.toolbar.get_menu(parent);if(menu.find("li:not(.custom-menu)").length&&!menu.find(".divider").length){frappe.ui.toolbar.add_menu_divider(menu);}
return $('<li class="custom-menu"><a><i class="icon-fixed-width '
+icon+'"></i> '+label+'</a></li>').insertBefore(menu.find(".divider")).find("a").click(function(){click.apply(this);});},get_menu:function(label){return $("#navbar-"+label.toLowerCase());},add_menu_divider:function(menu){menu=typeof menu=="string"?frappe.ui.toolbar.get_menu(menu):menu;$('<li class="divider custom-menu"></li>').prependTo(menu);},toggle_left_sidebar:function(){$(".offcanvas").toggleClass("active-left").removeClass("active-right");return false;}})
